...
export default function ServiceDetailPage() {
  ...
  const goToPartnerDesk = () => navigate("/", { state: { scrollTo: "sponsors" } });
  if (!service && services.length) return <div className="site-shell"><Header onOpenRegister={() => setRegisterOpen(true)} onOpenDashboard={() => setDashboardOpen(true)} /><main className="event-not-found container"><h1>We couldnÃ¢â‚¬â„¢t find that experience.</h1><Link className="btn btn-primary" to="/services"><ArrowLeft size={16} /> All experiences</Link></main><Footer /></div>;
  if (!service) return <div className="site-shell"><Header onOpenRegister={() => setRegisterOpen(true)} onOpenDashboard={() => setDashboardOpen(true)} /><main className="event-not-found container"><p>Loading experienceÃ¢â‚¬Â¦</p></main><Footer /></div>;
  return <div className="site-shell"><Header onOpenRegister={() => setRegisterOpen(true)} onOpenDashboard={() => setDashboardOpen(true)} />
    <main className="service-detail-page">
      <section className="service-detail-hero"><div className="container"><Link className="detail-back" to="/services"><ArrowLeft size={15} /> All experiences</Link><motion.div className="event-detail-image" initial={{ opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }}><img src={service.image} alt={service.title} /><div className="event-detail-overlay" /><div className="event-detail-heading">{service.badge && <span className="detail-badge">{service.badge}</span>}<h1>{service.title}</h1></div></motion.div></div></section>
      <section className="section"><div className="container service-detail-layout"><div className="detail-main"><div className="section-kicker">Overview</div><h2 className="section-title">A room designed for meaningful momentum.</h2><p className="detail-intro">{service.subtitle}</p><div className="detail-section"><h3>Our Offerings (Services)</h3><div className="detail-list">{service.details.map((item) => <div key={item}><CheckCircle2 size={19} /><span>{item}</span></div>)}</div></div><div className="service-highlights"><div><div className="section-kicker">Benefits</div><h3>Made for outcomes that last beyond the agenda.</h3></div><div>{service.details.slice(0, 3).map((item, index) => <article key={item}><strong>0{index + 1}</strong><span>{item}</span></article>)}</div></div><div className="service-gallery"><div className="section-kicker">Gallery</div><h3>Inside the experience.</h3><div className="service-gallery-grid">{(gallery.length ? gallery : [{ id: service.id, coverImage: service.image, eventTitle: service.title }]).map((item) => <img key={item.id} src={item.coverImage} alt={item.eventTitle || service.title} />)}</div></div></div><aside className="detail-sidebar"><div className="detail-cta-card"><span className="cta-icon"><Ticket /></span><small>Invitation-only experience</small><h2>Join {service.title}</h2><p>Tell us what you hope to achieve and weÃ¢â‚¬â„¢ll prepare your delegate request.</p><button className="btn btn-primary btn-full" onClick={goToPartnerDesk}>Register interest <ArrowRight size={16} /></button><div className="cta-note"> Takes about two minutes</div></div><div className="mt-6"><JoinCommunityCard onOpen={() => setCommunityOpen(true)} /></div></aside></div></section>
    ...
    </main><button className="service-sticky-register" onClick={goToPartnerDesk}><Ticket size={16} /> Register interest</button><Footer /><AIPitchGenerator isOpen={registerOpen} onClose={() => setRegisterOpen(false)} preselectedProperty={service.title} preselectedCity="" /><CommunityRegistrationModal isOpen={communityOpen} onClose={() => setCommunityOpen(false)} /><InquiryDashboard isOpen={dashboardOpen} onClose={() => setDashboardOpen(false)} onInquirySubmittedCounter={0} /></div>;
}
